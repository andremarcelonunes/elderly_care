...
def test_create_association_success(mocker):
    ...
    # Call the method
    result = crud_assisted.create_association(db_session, subscriber_id=1, assisted_id=2)
...
def test_create_association_exception(mocker):
    ...
    mock_query = mocker.Mock()
    mock_query.filter.return_value.one_or_none.side_effect = Exception("Unexpected error")
    mocker.patch.object(db_session, "query", return_value=mock_query)
